 ...
 */
public class WebSysUserAuthenticationListener implements AuthenticationListener {
	...
	public void onSuccess(AuthenticationToken token, AuthenticationInfo info) {
		...
		WebUserService.put(RequestContext.get().session().getId(), onlineSysUserEntity);
		//log
	...
	public void onLogout(PrincipalCollection principals) {
		...
		WebUserService.remove(SecurityUtils.getSubject().getSession(false).getId().toString());
		//log
		LogService logService = ConsumerProxyFactory.getInstance().proxy(LogService.class);
		SysLogEntity logEntity = new SysLogEntity();
		logEntity.setSysUserEntity(sysUserEntity);
