...
package cn.com.rebirth.knowledge.web.realm;
...
import cn.com.rebirth.knowledge.web.service.WebSystemService;
 ...
 * @author l.xue.nong
 */
public class WebExamJdbcRealm extends AuthorizingRealm {

	/** The system service. */
	@Resource
	private WebSystemService systemService;

	/**
	 * Instantiates a new exam jdbc realm.
	 */
	public WebExamJdbcRealm() {
		setName("WebExamJdbcRealm"); //This name must match the name in the User class's getPrincipals() method
	...
	protected Object getAuthorizationCacheKey(PrincipalCollection principals) {
		...
		if (httpSession != null) {
			...
			if (sessionId != null) {
				OnlineSysUserEntity onlineSysUserEntity = WebUserService.get(sessionId);
				if (onlineSysUserEntity == null) {
					...
					if (user != null) {
						...
						WebUserService.put(sessionId, onlineSysUserEntity);
						//log
